...
class Trainer(object):
    ...
    def train_batch(self):
        ...
        for b_idx, batch in bar:
            ...
            if self.updates_counter % self.params.eval_every_iter == 0:
                ...
                if result['acc_macro'] >= self.best_metric:
                    ...
                    self.test_best_metric = test_result['acc_macro']
                    self.not_improved_count = 1
                    if self.params.dataset != 'BioSNAP':
                        # logging.info('Test Performance Per Class:' + str(save_test_data) + 'in ' + str(time.time() - tic)+'s')
                        logging.info('Test Performance Per Class:' +'in ' + str(time.time() - tic)+'s')
                    else:
        ...
        if self.params.dataset != 'BioSNAP':
            ...
            return total_loss / b_idx, f1_macro, auc_pr, f1, eval_total_time
        else:
            return total_loss / b_idx, 0, 0, 0, eval_total_time
    ...
    def train(self):
        ...
        for epoch in range(1, self.params.num_epochs + 1):
            ...
            # 🔁 单轮训练
            loss, acc_macro, auc_pr, f1, eval_time = self.train_batch()
            ...
            # 📢 日志记录
            logging.info(
                f"Epoch {epoch} with loss: {loss:.4f}, training acc_macro: {acc_macro:.4f}, training auc_pr: {auc_pr:.4f}, "
                f"best validation acc_macro: {self.best_metric:.4f}, epoch time: {time_elapsed:.2f}s, eval time: {eval_time:.2f}s"
            )
        ...
        logging.info(f"✅ Total Time (including setup, etc.): {total_time:.2f}s")
