...
class Classifier_model(torch.nn.Module):
    ...
    def forward(self,sub_graphs):
        ...
        return scores
    ### dt get embedding
    def get_pair_embedding(self, sub_graphs):
        g = sub_graphs
        g_device = next(self.embedding_model.parameters()).device
        self.global_graph = self.global_graph.to(g_device)
        self.global_graph.ndata['h'] = self.embedding_model(self.global_graph)
        g.ndata['h'] = self.global_graph.nodes[g.ndata['idx']].data['h']
        g.ndata['repr'] = self.global_graph.nodes[g.ndata['idx']].data['repr']
        ...
        head_ids = (complete_graph.ndata['id'] == 1).nonzero().squeeze(1)
        tail_ids = (complete_graph.ndata['id'] == 2).nonzero().squeeze(1)

        head_hidden = gsl_hidden[head_ids]  # (batch_size, emb_dim)
        tail_hidden = gsl_hidden[tail_ids]  # (batch_size, emb_dim)

        # 药物对 embedding，可以是拼接，也可以是其他操作
        pair_emb = torch.cat([head_hidden, tail_hidden], dim=1)  # (batch_size, emb_dim * 2)

        return pair_emb
    def encode_node(self, sub_graphs):
        g = sub_graphs
        g_device = next(self.embedding_model.parameters()).device
        self.global_graph = self.global_graph.to(g_device)
        self.global_graph.ndata['h'] = self.embedding_model(self.global_graph)
