...
def process_dataset(params):
    params.db_path = os.path.join(params.main_dir, f'upload/data/KnowDDI/data/{params.dataset}/S0/digraph_hop_{params.hop}_{params.BKG_file_name}')
...
def save_test_embeddings(evaluator, save_path):
    ...
    with torch.no_grad():
        for batch in dataloader:
            data_pos, r_labels_pos, targets_pos = evaluator.move_batch_to_device(batch, evaluator.params.device)

            emb = evaluator.graph_classifier.encode_node(data_pos)
            all_embeddings.append(emb.cpu().numpy())

            if isinstance(r_labels_pos, list):
                r_labels_pos = [torch.tensor(x) for x in r_labels_pos]
                r_labels_pos = torch.stack(r_labels_pos)

            all_labels.extend(r_labels_pos.cpu().numpy())

    # 合并并保存
    final_emb = np.concatenate(all_embeddings, axis=0)
    final_emb = final_emb.reshape(final_emb.shape[0], -1)
    print("final_emb shape:", final_emb.shape)
    np.save(os.path.join(save_path, "db_embeddings_0.npy"), final_emb)
    np.save(os.path.join(save_path, "db_labels_0.npy"), np.array(all_labels))
...
def main(params):
    ...
    params.file_paths = {
        'train': os.path.join(params.main_dir, 'upload/data/KnowDDI/data/{}/S0/{}.txt'.format(params.dataset, params.train_file)),
        'valid': os.path.join(params.main_dir, 'upload/data/KnowDDI/data/{}/S0/{}.txt'.format(params.dataset, params.valid_file)),
        'test': os.path.join(params.main_dir, 'upload/data/KnowDDI/data/{}/S0/{}.txt'.format(params.dataset, params.test_file))
    }
    train_data, test_data = process_dataset(params)
...
if __name__ == '__main__':
    ...
    # global
    parser.add_argument('--seed', type=int, default=226, help="seeds for random initial")
    ...
    parser.add_argument("--num_epochs", "-ne", type=int, default=100,help="numer of epochs")
