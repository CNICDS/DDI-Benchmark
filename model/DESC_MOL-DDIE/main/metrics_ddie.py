...
if _has_sklearn:
    ...
    def ddie_compute_metrics(task_name, preds, labels, every_type=False):
        ...
        p,r,f,s = precision_recall_fscore_support(y_pred=preds, y_true=labels, average='micro')
        result = {
            "Precision": p,
            "Recall": r,
            "microF1": f
        ...
        p,r,f,s = precision_recall_fscore_support(y_pred=preds, y_true=labels, average='macro')
        result["macroPrecision"] = f
        result["macroRecall"] = r
        result["macroF1"] = f

        acc = accuracy_score(y_pred=preds, y_true=labels)
        result["Accuracy"] = acc
        kappa = cohen_kappa_score(y1=preds, y2=labels)
        result["Kappa"] = kappa
        result["confusion_matrix"] = confusion_matrix(y_true=labels, y_pred=preds)
        if every_type:
