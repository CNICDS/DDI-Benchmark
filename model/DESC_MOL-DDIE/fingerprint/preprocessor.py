...
def create_datasets(corpus_tsv_dir, database_tsv_dir, radius):
    ...
    if corpus_tsv_dir != 'none':
        corpus_train = create_dataset(os.path.join(corpus_tsv_dir, 'train_ddi.tsv'))
        corpus_dev = create_dataset(os.path.join(corpus_tsv_dir, 'test_ddi.tsv'))
        outputs_dict['corpus_train'] = corpus_train
        outputs_dict['corpus_dev'] = corpus_dev
    if database_tsv_dir != 'none':
        database_train = create_dataset(os.path.join(database_tsv_dir, 'train_ddi.tsv'))
        database_dev = create_dataset(os.path.join(database_tsv_dir, 'test_ddi.tsv'))
...
# np.save(os.path.join(output_dir, 'corpus_train'), outputs_dict['corpus_train'])
np.save(os.path.join(output_dir, 'corpus_train'), np.asarray(outputs_dict['corpus_train'], dtype=object))
np.save(os.path.join(output_dir, 'corpus_dev'), np.asarray(outputs_dict['corpus_dev'], dtype=object))
