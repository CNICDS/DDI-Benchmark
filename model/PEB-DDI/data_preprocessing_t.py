...
if 'drugbank' not in dp_dataset_name:
    ...
    for id, smi in zip(df_drugs_smiles['drugbank_id'], df_drugs_smiles['smiles']):
        mol = Chem.MolFromSmiles(smi.strip())
        if mol is None:
            print(f"[WARN] 无法解析 SMILES，跳过: {id} -> {smi}")
            continue
        drug_id_mol_graph_tup.append((id, mol))
    # [(id, Chem.MolFromSmiles(smiles.strip())) for id, smiles in zip(df_drugs_smiles['drugbank_id'], df_drugs_smiles['smiles'])]
    ...
    drug_id_mol_graph_tup = []
    for id, smi in zip(df_drugs_smiles['drug_id'], df_drugs_smiles['smiles']):
        mol = Chem.MolFromSmiles(smi.strip())
        if mol is None:
            print(f"[WARN] 无法解析 SMILES，跳过: {id} -> {smi}")
            continue
...
n = fp.shape[0] 
for i in range(n):
    drug_id = DRUG_INDX_NAME_DICT[i]
    id_fp[drug_id]   = fp[i, :]
    id_desc[drug_id] = desc[i, :]
if len(DRUG_INDX_NAME_DICT) > n:
    missing = [DRUG_INDX_NAME_DICT[i] for i in range(n, len(DRUG_INDX_NAME_DICT))]
    print(f"[WARN] {len(missing)} drug(s) have no fingerprint: {missing[:5]} …")
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        if 'drugbank' not in dp_dataset_name:
            for h, t, r in batch:
                if r == 1 and h in id_fp and t in id_fp:
                    # pos_h
                    pos_rels.append(r)
                ...
                if r == 0:
                    # neg_h
                    neg_rels.append(r)
                    if h not in id_fp or t not in id_fp:
                        continue
            ...
            for h, t, r in batch:
                if  h in id_fp and t in id_fp:
                    ...
                    for neg_h in neg_heads:
                        if neg_h not in id_fp or t not in id_fp:
                            continue
                        neg_rels.append(r)
                    ...
                    for neg_t in neg_tails:
                        if h not in id_fp or neg_t not in id_fp:
                            continue
                        neg_rels.append(r)
                        neg_h_samples.append(self.__create_graph_data(h))
                        neg_h_fp.append(id_fp[h])
