...
class Args(Tap):
    data_path: str='/home/chenzidan/DB_new/dsn/ddis_smiles.csv'  # Path to data CSV
    ...
    save_path: str='./features/db2025.npz'  # Path to .npz file where features will be saved as a compressed numpy archive
...
def safe_features_generator(smiles_str):
    """
    Wrap the real generator to catch any Boost.Python (or other) exceptions
    so they don't escape the worker process.
    """
    try:
        return features_generator(smiles_str)
    except Exception as e:
        # 这里打印一下是哪条 SMILES 异常了，方便排查
        print(f"[WARN] Feature generation failed for {smiles_str}: {e}")
        return None
...
def generate_and_save_features(args: Args):
    ...
    if args.sequential or True:
        features_map = map(features_generator, smiles)
    else:
        # Use safe wrapper in parallel workers
        pool = Pool()
        features_map = pool.imap(safe_features_generator, smiles)
    ...
    try:
        ...
        save_features(args.save_path, features_arr)
