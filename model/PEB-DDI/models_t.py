...
# dt add
    ...
    #     _, _, h_data_fin, h_data_desc, t_data_fin, t_data_desc = self.fdmer(
    #         h_data_fin, h_data_desc, t_data_fin, t_data_desc
    #     )

    #     repr_h_fd = [F.normalize(repr_h[i] + h_data_fin) for i in range(len(self.blocks))]
    #     repr_t_fd = [F.normalize(repr_t[i] + t_data_fin) for i in range(len(self.blocks))]

    #     kge_heads = torch.stack(repr_h_fd, dim=1)
    #     kge_tails = torch.stack(repr_t_fd, dim=1)

    #     attentions = self.co_attention(kge_heads, kge_tails)

    #     h_embed = torch.sum(kge_heads * attentions, dim=1)
    #     t_embed = torch.sum(kge_tails * attentions, dim=1)

    #     pair_embedding = torch.cat([h_embed, t_embed], dim=-1)
    ...
    def encode_pair(self, triples):
        ...
        repr_h = torch.stack(repr_h_fd, dim=1)  # [B, L, D]
        repr_t = torch.stack(repr_t_fd, dim=1)  # [B, L, D]

        # === 取最后一层（或平均） ===
        final_h = repr_h[:, -1, :]  # [B, D]
        final_t = repr_t[:, -1, :]  # [B, D]

        drug_pair_embedding = torch.cat([final_h, final_t], dim=-1)  # [B, 2D]
