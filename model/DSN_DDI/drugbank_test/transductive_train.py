...
parser.add_argument('--pkl_name', type=str, default='s0_226_db.pkl')
...
df_ddi_train = pd.read_csv('./data/Drugbank/train_s0.csv')
df_ddi_val = pd.read_csv('./data/Drugbank/dsn/valid_s0.csv')
df_ddi_test = pd.read_csv('./data/Drugbank/test_s0.csv')
...
val_data_loader = DrugDataLoader(val_data, batch_size=batch_size *3,num_workers=0)
test_data_loader = DrugDataLoader(test_data, batch_size=batch_size *3,num_workers=0)
...
def train(model, train_data_loader, val_data_loader, loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        with open(result_name, 'a+') as f:
            f.write(f'Epoch: {i} ({time.time() - start:.4f}s), train_loss: {train_loss:.4f}, val_loss: {val_loss:.4f},train_acc: {train_acc:.4f}, val_acc:{val_acc:.4f}, train_f1: {train_f1:.4f}, val_f1:{val_f1:.4f}\n')
            f.write(f'\t\ttrain_roc: {train_auc_roc:.4f}, val_roc: {val_auc_roc:.4f}, train_precision: {train_precision:.4f}, val_precision: {val_precision:.4f}\n')
...
def test(test_data_loader,model):
    ...
    with open(result_name, 'a+') as f:
        f.write('============================== Test Result ==============================\n')
        f.write(f'\t\ttest_acc: {test_acc:.4f}, test_auc_roc: {test_auc_roc:.4f},test_f1: {test_f1:.4f},test_precision:{test_precision:.4f}\n')
        f.write(f'\t\ttest_recall: {test_recall:.4f}, test_int_ap: {test_int_ap:.4f},test_ap: {test_ap:.4f}\n')
