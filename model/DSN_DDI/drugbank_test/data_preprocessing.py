...
valid_drug_id_mol_graph_tup = []
invalid_smiles = []

for id, smiles in zip(df_drugs_smiles['drug_id'], df_drugs_smiles['smiles']):
    mol = Chem.MolFromSmiles(smiles.strip())
    if mol is None:
        invalid_smiles.append((id, smiles))
    else:
        valid_drug_id_mol_graph_tup.append((id, mol))

# Log invalid SMILES
print("Invalid SMILES strings:")
for id, smiles in invalid_smiles:
    print(f"Drug ID: {id}, SMILES: {smiles}")

# Create a dictionary of valid molecules
drug_to_mol_graph = {id: mol for id, mol in valid_drug_id_mol_graph_tup}

# Calculate atom-related properties
ATOM_MAX_NUM = np.max([mol.GetNumAtoms() for _, mol in valid_drug_id_mol_graph_tup])
AVAILABLE_ATOM_SYMBOLS = list({a.GetSymbol() for a in itertools.chain.from_iterable(mol.GetAtoms() for _, mol in valid_drug_id_mol_graph_tup)})
AVAILABLE_ATOM_DEGREES = list({a.GetDegree() for a in itertools.chain.from_iterable(mol.GetAtoms() for _, mol in valid_drug_id_mol_graph_tup)})
AVAILABLE_ATOM_TOTAL_HS = list({a.GetTotalNumHs() for a in itertools.chain.from_iterable(mol.GetAtoms() for _, mol in valid_drug_id_mol_graph_tup)})
max_valence = max(a.GetImplicitValence() for a in itertools.chain.from_iterable(mol.GetAtoms() for _, mol in valid_drug_id_mol_graph_tup))
AVAILABLE_ATOM_VALENCE = np.arange(max(max_valence, 9) + 1)

MAX_ATOM_FC = abs(np.max([a.GetFormalCharge() for a in itertools.chain.from_iterable(mol.GetAtoms() for _, mol in valid_drug_id_mol_graph_tup)]))
MAX_RADICAL_ELC = abs(np.max([a.GetNumRadicalElectrons() for a in itertools.chain.from_iterable(mol.GetAtoms() for _, mol in valid_drug_id_mol_graph_tup)]))
...
MOL_EDGE_LIST_FEAT_MTX = {drug_id: get_mol_edge_list_and_feat_mtx(mol) 
                                for drug_id, mol in valid_drug_id_mol_graph_tup}
...
df_all_pos_ddi = pd.read_csv('./data/Drugbank/ddis_processed.csv')
all_pos_tup = [(h, t, r) for h, t, r in zip(df_all_pos_ddi['d1'], df_all_pos_ddi['d2'], df_all_pos_ddi['type'])]


ALL_DRUG_IDS, _ = zip(*valid_drug_id_mol_graph_tup)
