...
class InterGraphAttention(nn.Module):
    ...
    #     return h_rep,t_rep
    def forward(self,h_data,t_data,b_graph, return_attention=False):
        edge_index = b_graph.edge_index
        h_input = F.elu(h_data.x)
        t_input = F.elu(t_data.x)
        
        if return_attention:
            t_rep, (t_edge_index, t_att_weights) = self.inter((h_input, t_input), edge_index, return_attention_weights=True)
            h_rep, (h_edge_index, h_att_weights) = self.inter((t_input, h_input), edge_index[[1,0]], return_attention_weights=True)
            return h_rep, t_rep, (h_edge_index, h_att_weights), (t_edge_index, t_att_weights)
        else:
            t_rep = self.inter((h_input, t_input), edge_index)
            h_rep = self.inter((t_input, h_input), edge_index[[1,0]])
            return h_rep, t_rep
