...
class MVN_DDI(nn.Module):
    ...
    # def encode_pair(self, triples):
    #     """
    #     获取每对药物交互后的 embedding，用于下游任务或可视化。
    #     使用 h_final 与 t_final 的 concat 作为药物对的最终表示。
        
    #     返回：
    #         - pair_embedding: (batch_size, kge_dim * 2)
    #     """
    #     self.eval()
    #     h_data, t_data, rels, b_graph = triples
    ...
    #     repr_h = torch.stack(repr_h, dim=-2)  # shape: (batch_size, dim, n_blocks)
    #     repr_t = torch.stack(repr_t, dim=-2)

    #     # attentions = self.co_attention(repr_h, repr_t)  # (batch_size, n_blocks, 1)

    #     # # 变形：转成 (batch_size, n_blocks, dim)
    #     # repr_h = repr_h.transpose(1, 2)
    #     # repr_t = repr_t.transpose(1, 2)

    #     # h_final = torch.sum(repr_h * attentions, dim=1)  # (batch_size, dim)
    #     # t_final = torch.sum(repr_t * attentions, dim=1)

    #     pair_embedding = torch.cat([repr_h, repr_t], dim=-1)  # (batch_size, dim * 2)

    #     return pair_embedding  # 可以选择同时返回 attentions 等信息
    def encode_pair(self, triples):
        """
        提取药物对的embedding。
        ...
        返回:
            drug_pair_embeddings: [batch_size, embedding_dim]
        """
        h_data, t_data, rels, b_graph = triples
        ...
        # [batch, layers, dim]
        repr_h = torch.stack(repr_h, dim=1)  # dim=-2变成dim=1更常见
        repr_t = torch.stack(repr_t, dim=1)

        final_h = repr_h[:, -1, :]  # 最后一层
        final_t = repr_t[:, -1, :]

        # 可选：拼接头尾作为pair的embedding
        drug_pair_embedding = torch.cat([final_h, final_t], dim=-1)  # [batch, 2*dim]
        return drug_pair_embedding
    def encode_node(self, triples):
        """
        提取药物对的embedding。
...
class MVN_DDI_Block(nn.Module):
    ...
    def forward(self, h_data,t_data,b_graph):
        ...
        h_interRep, t_interRep, h_att_info, t_att_info = self.interAtt(h_data, t_data, b_graph, return_attention=True)
