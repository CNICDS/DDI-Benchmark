...
df_ddi_test = pd.read_csv('./data/Drugbank/virtual/test_s1.csv')
...
def extract_embeddings(model, test_loader, save_path_emb, device=device):
    model.eval()
    all_embeddings = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            for tri, label_value in [(pos_tri, 1)]:
                ...
                try:
                    emb = model.encode_node((h_data, t_data, rels, b_graph))  # shape: [batch_size, emb_dim]
...
extract_embeddings(test_model, test_data_loader,
                     save_path_emb='saved_embeddings/db_nodeembeddings_1_try.npy',device=device)
