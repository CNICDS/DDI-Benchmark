...
parser.add_argument('--rel_total', type=int, default=200, help='num of interaction types')
...
parser.add_argument('--pkl_name', type=str, default='s1s2_226_tw.pkl')
parser.add_argument('--result_name', type=str, default='./model/DSN_DDI/twosides_test/result/s1s2_226_tw.txt')
...
###### Dataset
df_ddi_train = pd.read_csv('./data/twosides/train_s1.csv')
df_ddi_s1 = pd.read_csv('./data/twosides/test_s1.csv')
df_ddi_s2 = pd.read_csv('./data/twosides/test_s2.csv')


train_tup = [(h, t, r, n) for h, t, r, n in zip(df_ddi_train['d1'], df_ddi_train['d2'], df_ddi_train['type'], df_ddi_train['Neg samples'])]
s1_tup = [(h, t, r, n) for h, t, r, n in zip(df_ddi_s1['d1'], df_ddi_s1['d2'], df_ddi_s1['type'], df_ddi_s1['Neg samples'])]
s2_tup = [(h, t, r, n) for h, t, r, n in zip(df_ddi_s2['d1'], df_ddi_s2['d2'], df_ddi_s2['type'],df_ddi_s2['Neg samples'])]

train_data = DrugDataset(train_tup)
s1_data = DrugDataset(s1_tup)
s2_data = DrugDataset(s2_tup)
...
def train(model, train_data_loader, s1_data_loader, s2_data_loader, loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        print(f'\t\ts1_acc: {s1_acc:.4f}, s1_roc: {s1_auc_roc:.4f}, s1_precision: {s1_precision:.4f}, s1_recall: {s1_recall:.4f},s1_f1: {s1_f1:.4f}')
        print(f'\t\ts2_acc: {s2_acc:.4f}, s2_roc: {s2_auc_roc:.4f}, s2_precision: {s2_precision:.4f}, s2_recall: {s2_recall:.4f},s2_f1: {s2_f1:.4f}')
        with open(result_name, 'a+') as f:
            f.write(f'Epoch: {i} ({time.time() - start:.4f}s), train_loss: {train_loss:.4f}, s1_loss: {s1_loss:.4f},s2_loss: {s2_loss:.4f}\n')
            f.write(f'\t\ttrain_acc: {train_acc:.4f}, train_roc: {train_auc_roc:.4f},train_precision: {train_precision:.4f},train_recall:{train_recall:.4f}\n')
            f.write(f'\t\ts1_acc: {s1_acc:.4f}, s1_roc: {s1_auc_roc:.4f}, s1_precision: {s1_precision:.4f}, s1_recall: {s1_recall:.4f},s1_f1: {s1_f1:.4f}\n')
            f.write(f'\t\ts2_acc: {s2_acc:.4f}, s2_roc: {s2_auc_roc:.4f}, s2_precision: {s2_precision:.4f}, s2_recall: {s2_recall:.4f},s2_f1: {s2_f1:.4f}\n')
...
def test(s1_data_loader, s2_data_loader, model):
    ...
    print('\n')
    print('============================== Best Result ==============================')
    print(f'\t\ts1_acc: {s1_acc:.4f}, s1_roc: {s1_auc_roc:.4f}, s1_f1: {s1_f1:.4f}, s1_precision: {s1_precision:.4f},s1_recall: {s1_recall:.4f},s1_f1: {s1_f1:.4f},s1_int_ap: {s1_int_ap:.4f},s1_ap: {s1_ap:.4f}\n')
    print(f'\t\ts2_acc: {s2_acc:.4f}, s2_roc: {s2_auc_roc:.4f}, s2_f1: {s2_f1:.4f}, s2_precision: {s2_precision:.4f},s2_recall: {s2_recall:.4f},s2_f1: {s2_f1:.4f},s2_ap: {s2_ap:.4f}\n')
    with open(result_name, 'a+') as f:
        f.write('============================== Best Result ==============================\n')
        f.write(f'\t\ts1_acc: {s1_acc:.4f}, s1_roc: {s1_auc_roc:.4f}, s1_f1: {s1_f1:.4f}, s1_precision: {s1_precision:.4f},s1_recall: {s1_recall:.4f},s1_f1: {s1_f1:.4f},s1_int_ap: {s1_int_ap:.4f},s1_ap: {s1_ap:.4f}\n')
        f.write(f'\t\ts2_acc: {s2_acc:.4f}, s2_roc: {s2_auc_roc:.4f}, s2_f1: {s2_f1:.4f}, s2_precision: {s2_precision:.4f},s2_recall: {s2_recall:.4f},s1_f1: {s1_f1:.4f},s2_int_ap: {s2_int_ap:.4f},s2_ap: {s2_ap:.4f}\n')
