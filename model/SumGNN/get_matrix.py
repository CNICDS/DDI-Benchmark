...
def save_confusion_matrix(params, graph_classifier, data, save_dir='results', dataset_name='default', num_classes=86):
    pos_scores = []
    pos_labels = []
    ...
    with torch.no_grad():
        for b_idx, batch in enumerate(dataloader):
            data_pos, r_labels_pos, targets_pos = params.move_batch_to_device(batch, params.device)
    ...
    # ✅ 确定标签列表
    # 建议使用全量标签（如0~85）以固定维度
    labels = list(range(num_classes))

    # ✅ 混淆矩阵计算（固定标签顺序）
    cm = confusion_matrix(pos_labels, pos_scores, labels=labels)
    print("Confusion Matrix shape:", cm.shape)

    # ✅ 保存混淆矩阵和标签
    os.makedirs(save_dir, exist_ok=True)

    cm_path = os.path.join(save_dir, f'confusion_matrix_{dataset_name}.pkl')
    labels_path = os.path.join(save_dir, f'confusion_matrix_{dataset_name}_labels.pkl')
    ...
    print(f"Label index list saved to: {labels_path}")
...
def main(params):
    ...
    # test_evaluator.eval()  # 里面已生成 pred_class 字典
    save_confusion_matrix(params,graph_classifier, test,save_dir='saved_results', dataset_name=params.dataset)
