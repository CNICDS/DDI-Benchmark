...
def save_test_embeddings(evaluator, save_path):
    os.makedirs(save_path, exist_ok=True)

    dataloader = DataLoader(
        evaluator.data,
        batch_size=evaluator.params.batch_size,
        shuffle=False,
        num_workers=evaluator.params.num_workers,
        collate_fn=evaluator.params.collate_fn
    )
    # print(type(evaluator.data), len(evaluator.data))
    # print("Sample label from evaluator.data[0]:", evaluator.data[0])

    evaluator.graph_classifier.eval()
    all_embeddings = []
    all_labels = []

    with torch.no_grad():
        for batch in dataloader:
            data_pos, r_labels_pos, targets_pos = evaluator.params.move_batch_to_device(batch, evaluator.params.device)

            # print("targets_pos example:", targets_pos[:5])  # 👈 看看是否都是 1
            # print("r_labels_pos example:", r_labels_pos[:5])
            emb = evaluator.graph_classifier.encode_node(data_pos)

            all_embeddings.append(emb.cpu().numpy())
            all_labels.extend(r_labels_pos.cpu().numpy())  

    # 合并并保存
    final_emb = np.concatenate(all_embeddings, axis=0)
    print(f"Final embeddings shape: {final_emb.shape}")
    np.save(os.path.join(save_path, "dbn_nodeembeddings_2.npy"), final_emb)
    np.save(os.path.join(save_path, "dbn_nodelabels_2.npy"), np.array(all_labels))
...
def main(params):
    ...
    if params.dataset == 'drugbank':
        if params.feat == 'morgan':
            import pickle 
            with open('upload/data/SumGNN/{}/S0/DB_molecular_feats.pkl'.format(params.dataset), 'rb') as f:
            ...
            mfeat = np.loadtxt('upload/data/SumGNN/{}/S0/PCA.txt'.format(params.dataset))
            params.feat_dim = 200
        elif  params.feat == 'pretrained':
            mfeat = np.loadtxt('upload/data/SumGNN/{}/S0/pretrained.txt'.format(params.dataset))
            params.feat_dim = 200
    elif params.dataset == 'BioSNAP':
        mfeat = []
        rfeat = []
        import pickle 
        with open('upload/data/SumGNN/{}/S0/id2drug_feat.pkl'.format(params.dataset), 'rb') as f:
            x = pickle.load(f, encoding='utf-8') 
        for z in x:
            y = x[z]['Morgan']
            mfeat.append(y)
    ...
    # === 加载模型 & 保存 test 的 embedding ===
    save_path = os.path.join(params.main_dir, "saved_embeddings")
    save_test_embeddings(test_evaluator, save_path)
...
if __name__ == '__main__':
    ...
    random.seed(wx_seed)
    # 设置 numpy 的随机种子
    np.random.seed(wx_seed)
    # 设置 PyTorch 的随机种子
    torch.manual_seed(wx_seed)

if torch.cuda.is_available():
    torch.cuda.manual_seed(wx_seed)
    torch.cuda.manual_seed_all(wx_seed)
    ...
    params.file_paths = {
        'train': os.path.join(params.main_dir, 'upload/data/SumGNN/{}/S0/{}.txt'.format(params.dataset, params.train_file)),
        'valid': os.path.join(params.main_dir, 'upload/data/SumGNN/{}/S0/{}.txt'.format(params.dataset, params.valid_file)),
        'test': os.path.join(params.main_dir, 'upload/data/SumGNN/{}/S0/{}.txt'.format(params.dataset, params.test_file))
    }
