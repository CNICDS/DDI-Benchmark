...
class AutoModel(object):
    ...
    def derive_target_model(self):
        ...
        performance_list = []
        base_dir = os.path.split(os.path.realpath(__file__))[0][:-(len('autoddi')+1)]
        log_dir = os.path.join(base_dir, "logger", str(self.data.data_logger_save))
        ...
        log_file_path = os.path.join(log_dir, f"gnn_logger_{timestamp}.txt")
        for target_architecture, num in zip(sorted_gnn_architecture_list, model_num):
            ...
            with open(log_file_path, "a") as f_log:
                f_log.write(f"test gnn architecture:\t{target_architecture}\n")
            performance_all = []
            for fold in folds:
            ...
            if self.data.transductive_flag:
                ...
                with open(log_file_path, "a") as f_log:
                    f_log.write(f"all fold test_acc:\t{round(test_acc.mean().item(), 2)}±{round(test_acc.std().item(), 2)}\n")
                    f_log.write(f"all fold test_auc_roc:\t{round(test_auc_roc.mean().item(), 2)}±{round(test_auc_roc.std().item(), 2)}\n")
                    f_log.write(f"all fold test_f1:\t{round(test_f1.mean().item(), 2)}±{round(test_f1.std().item(), 2)}\n")
                    f_log.write(f"all fold test_ap:\t{round(test_ap.mean().item(), 2)}±{round(test_ap.std().item(), 2)}\n")
