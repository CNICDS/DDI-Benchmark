...
class drugbankDataset(InMemoryDataset):
    def __init__(self, root='/tmp',
        ...
        self.processed_path = os.path.join(os.path.dirname(path), os.path.basename(path).replace('.csv', '.pt'))
        ...
        if os.path.isfile(self.processed_path):
            print('Pre-processed data found: {}, loading ...'.format(self.processed_path))
            self.data, self.slices = torch.load(self.processed_path, weights_only=False)
        else:
            print('Pre-processed data {} not found, doing pre-processing...'.format(self.processed_path))
            self.process(root)
            self.data, self.slices = torch.load(self.processed_path, weights_only=False)
    ...
    def process(self, root):
        ...
        for i in range(data_len):
            ...
            if random.random() > 0.5:  # 换尾
                ...
                pes_tail = random.sample(list(id_set - set(tail_set)), 1)
                smile2 = id_smiles_dict[pes_tail[0]]
            else:
                tail = df1.loc[i, 'Drug2_ID']
                head_set = t_to_h_dict[tail]
                pes_head = random.sample(list(id_set - set(head_set)), 1)
