...
def main():
    ...
    parser.add_argument('--seed', default=0, type=int, required=False,
                        help='..')
    parser.add_argument('--dataset_name', default="drugbank", type=str, required=False,
                        help='..')
    ...
    dataset_path = f"upload/data/3DGT-DDI/drugbank2025/{args.dataset_name}"
    train_dataset = drugbankDataset(root=args.drugbank_root, path=os.path.join(dataset_path, "train_ddi.csv"))
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, follow_batch=['pos1', 'pos2'])
    valid_dataset = drugbankDataset(root=args.drugbank_root, path=os.path.join(dataset_path, "valid_ddi.csv"))
    val_loader = DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=True, follow_batch=['pos1', 'pos2'])
    log_path = f"upload/model/3DGT-DDI/logs/drugbank2025/{args.dataset_name}/{args.seed}"
    os.makedirs(log_path, exist_ok=True)
    model = train_eval_drugbank(model, optimizer, train_loader, val_loader, args.epochs , log_path)

    test_dataset = drugbankDataset(root=args.drugbank_root, path=os.path.join(dataset_path, "test_ddi.csv"))
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, follow_batch=['pos1', 'pos2'])

    label, pred, prob = get_label_pred_prob(model, test_loader)
    ...
    confusion_matrix = sklearn.metrics.confusion_matrix(label, pred)
    pd.DataFrame(confusion_matrix).to_csv(os.path.join(log_path, "confusion_matrix.csv"), index=False)
