...
def init_args(user_args=None):
    ...
    parser.add_argument('--dataset', type=str, default="twosidesS2")  # drugbank_new0, drugbank_new1, drugbank_new2, drugbank_new3, twosides, twosides_1, twosides_2
    ...
    parser.add_argument('--load_model_path',type=str,default='best_save/tiger/twosidesS2/randomWalk/11/0.48509/TIGER.pt')
...
def extract_embeddings(model, test_loader, device, save_emb='embeddings.npy'):
    model.eval()
    embedding_list = []
    label_list = []
    ...
    with torch.no_grad():
        for data in tqdm(test_loader, ncols=80):
            ...
            # 获取药物对的 embedding
            emb = model.encode_pair(data_mol1, data_drug1, data_mol2, data_drug2)  # [B, 128]
    ...
    emb_arr = np.concatenate(embedding_list, axis=0)
    # label_arr = np.concatenate(label_list, axis=0)

    np.save(save_emb, emb_arr)
    # np.save(save_lab, label_arr)

    print(f'✅ embeddings saved to: {save_emb} shape={emb_arr.shape}')
    # print(f'✅ labels saved to: {save_lab} shape={label_arr.shape}')
...
def main(args=None):
    ...
    extract_embeddings(model, test_loader, device,
                  save_emb='tw_nodeembeddings_2.npy')
