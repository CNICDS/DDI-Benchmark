...
#     print(f"drug_dict 大小: {len(drug_dict)}")
# # 打印字典的前几个项
#     for key, value in list(drug_dict.items())[:10]:
#         print(f"{key}: {value}")
...
def read_network(path):
    ...
    # num_rel = max(rel_index) + 1
        ...
        # 处理空的 edge_index 和 rel_index 情况  
    if edge_index:  
        num_node = np.max(np.array(edge_index))  
    else:  
        num_node = 0  # 或者设置为其它合适的默认值  

    if rel_index:  
        num_rel = max(rel_index) + 1  
        print(len(set(rel_index)))  # 打印唯一关系数  
    else:  
        num_rel = 0  # 或者设置为其它合适的默认值  
...
def generate_node_subgraphs(dataset, drug_id, network_edge_index, network_rel_index, num_rel, args):
    ...
    # todo
    # 检查 edge_index 和 rel_index 是否为空  
    if not network_edge_index or not network_rel_index:  
        print("Network edge index or relation index is empty. Skipping subgraph generation.")  
        return [], 0, 0  # 返回空的结果或其他默认值，避免后续逻辑操作  
    ...
    # todo
    # 确保 edge_index 符合期望的维度  
    if edge_index.size(1) == 0:  
        print("Edge index is empty after conversion to tensor. Skipping further processing.")  
        return [], 0, 0  # 提供返回的默认结果  
