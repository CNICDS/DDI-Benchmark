...
class DTADataset(InMemoryDataset):
    def __init__(self, x=None, y=None, sub_graph=None, smile_graph=None, ):
        ...
        for i in range(len(x)):
            d1, d2 = str(x[i][0]), str(x[i][1])
            if d1 in smile_graph and d2 in smile_graph:
                self.drug_ID.append(x[i])
    ...
    def read_drug_info(self, drug_id, labels):
        ...
        # todo
            # 检查sub_graph中是否有对应的drug_id  
        if str(drug_id) in self.sub_graph:  
            subset, subgraph_edge_index, subgraph_rel, mapping_id, s_edge_index, s_value, s_rel, deg = self.sub_graph[str(drug_id)]  
        else:  
            # 如果子图不存在，设置默认值，确保兼容操作  
            subset = np.array([])  # 空数组，仅在data_graph中用作x的输入  
            subgraph_edge_index = np.array([[], []], dtype=int)  # 使用空的2D array避免转置错误  
            subgraph_rel = np.array([], dtype=int)  
            mapping_id = np.array([], dtype=bool)  
            s_edge_index = np.array([[], []], dtype=int)  # 空的2D array  
            s_value = np.array([], dtype=int)  
            s_rel = np.array([], dtype=int)  
            deg = 0  
            # print(f"Warning: No subgraph found for drug_id {drug_id}. Using empty subgraph.")  
