    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(zip(*split_fold(k_fold, data, labels, args.s_type))):
        ...
        if args.model_name:
            ...
            for i_episode in range(args.model_episodes):
                ...
                # todo 输出所有指标
                print(f"Train - Acc: {train_acc}, F1: {train_f1}, AUC: {train_auc}, AUPR: {train_aupr}, Loss: {train_loss}")  
                print(f"Eval - Acc: {eval_acc}, F1: {eval_f1}, AUC: {eval_auc}, AUPR: {eval_aupr}, Loss: {eval_loss}")  
