...
class GraphTransformer(torch.nn.Module):
    ...
    def forward(self, feature, data, return_all_layers=False):
        x = feature
        graph_embedding_layer = []  # 每层的节点嵌入
        attn_layer = []             # 每层的注意力
        ...
        # 构造每层的图级表示（全局平均池化）
        if self.type == 'graph':
            pooled_reprs = [global_mean_pool(layer_x, batch=data.batch) for layer_x in graph_embedding_layer]
            sub_representation = []
            for index, drug_mol_graph in enumerate(data.to_data_list()):
                sub_embedding = x[(data.batch == index).nonzero().flatten()]
                sub_representation.append(sub_embedding)
        else:
            pooled_reprs = [layer_x[data.id.nonzero().flatten()] for layer_x in graph_embedding_layer]
            sub_representation = []
            for index, drug_subgraph in enumerate(data.to_data_list()):
                sub_embedding = x[(data.batch == index).nonzero().flatten()]
                sub_representation.append(sub_embedding)

        final_representation = pooled_reprs[-1]  # 最后一层的图表示

        if return_all_layers:
            # 返回所有层的图表示（List[Tensor]），形状：(num_layers, batch_size, emb_dim)
            return pooled_reprs, sub_representation, attn_layer
        else:
            return final_representation, sub_representation, attn_layer
